			...
			RPC.exec('im.getMediaInfo', {path : p}, function (data) {
				...
				switch (row.type) {
					...
					case "swf":
						...
						swfobject.embedSWF("../../stream/default.aspx?cmd=im.streamFile&path=" + escape(row.path), "flash", row.width, row.height, "7", null, {
						}, {
							quality : "high",
							wmode : "transparent",
							scale : "showall"
					...
					case "flv":
						...
						swfobject.embedSWF("flvplayer/flvPlayer.swf", "flash", row.width, row.height, "8", null, {
							flvToPlay : escape("../../../stream/default.aspx?cmd=im.streamFile&path=" + escape(row.path)),
							hiddenGui : "false",
							showScaleModes : "true",
							autoStart : "false",
